...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (postId) {
      ...
      setError(null);
      fetchData(
        `https://almahdicentre.org/wp-json/tribe/events/v1/events/${postId}`,
        setPost
      );
    } else {
      ...
      <ScrollView>
        <View>
          <View style={eventStyles.sectionHeader}>
            <Text style={eventStyles.title}>Upcoming Events</Text>
          </View>
          <View style={eventStyles.eventContent}>
            {post && (
              <View key={post.id} style={eventStyles.post}>
                ...
                <Text style={eventStyles.detailsTitle}>DETAILS</Text>
                <Text style={eventStyles.postDate}>
                  Date: {months[post.start_date_details.month - 1]} ,{" "}
                  {formatMonth(post.start_date_details.day)}
                </Text>
                <Text style={eventStyles.postDate}>
                  Time: {post.start_date_details.hour}:
                  {post.start_date_details.minutes} -{" "}
                  {post.end_date_details.hour}:{post.end_date_details.minutes}
                </Text>
