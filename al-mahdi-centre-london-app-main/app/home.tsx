...
let PrayerNotifications: any;


function openPortal() {
  Linking.openURL("https://almahdi.breezechms.com/login");
}

export const SectionHeader = ({ icon, title }) => (
  <View style={styles.sectionHeader}>
    <Text style={styles.title}>
      <Icon name={icon} size={22} color="#4b5568" /> {title}
    </Text>
  </View>
);
...
export default function HomeScreen() {
  ...
  const fetchIslamicDate = async () => {
    try {
      const response = await fetch(
        "https://almahdicentre.org/wp-json/simple-csv-json/v1/data"
      );
  ...
  const fetchPostsAndEvents = async () => {
    fetchData(
      "https://almahdicentre.org/wp-json/wp/v2/posts",
      setPosts,
      setLoading,
      setError
    );
    fetchData(
      "https://almahdicentre.org/wp-json/tribe/events/v1/events",
      (data) => {
              ...
              events.slice(0, 5).map((event) => (
                ...
                >
                  <EventItem
                    ...
                    date={`${formatEventStartDate(event.start_date)}`}
                  />
                </Pressable>
              ))
            ) : (
...
async function fetchPrayerTimes() {
  try {
    const response = await fetch("https://almahdicentre.org/wp-json/simple-csv-json/v1/data");
    const data = await response.json();
...
export async function schedulePrayerNotifications() {
  ...
  const savedSwitchStates = savedSwitchStatesRaw ? JSON.parse(savedSwitchStatesRaw) : {};
  ...
  for (const prayer of prayerTimes) {
    ...
    for (const key of Object.keys(prayerTimesForDay)) {
      if (key !== "Date" && key !== "Imsak" && key !== "Sunrise" && key !== "Sunset" && counter <= 50) { // Skip the date key
        ...
        if (scheduledDateTime > now) {
          ...
          await Notifications.scheduleNotificationAsync({
            ...
            trigger: { 
              ...
              channelId: savedSwitchStates[key] === "Adhaan" ? "prayer_times_adhaan" : "prayer_times",
            },
          });

          console.log(`Scheduled notification for ${key} at ${scheduledDateTime}`);
          counter++;
